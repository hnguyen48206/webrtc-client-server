...
const nodewrtc = require('./nodewrtc')
const app = express();

const port = process.env.PORT || 9000;
global.serverUsers = [];
global.ffmpegStreamer = [];
...
wss.on("connection", ws => {
  ws.on("message", msg => {
    ...
    try {
      data = JSON.parse(msg);
      // console.warn('Dta nhận được từ CLIENT ', data)
    } catch (e) {
  ...
  ws.send(
    JSON.stringify({
      type: "connect",
      message: "This is the mss from server. Connection is opened."
    })
  );
});
